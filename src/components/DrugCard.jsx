...
export default function DrugCard({ drug }) {
  ...
  return (
    <button
      ...
      className="bg-white dark:bg-[#111827] rounded-2xl p-4 sm:p-5 shadow-[0_1px_3px_rgba(0,0,0,0.03)] hover:shadow-[0_10px_25px_-5px_rgba(0,0,0,0.06)] dark:hover:shadow-[0_10px_25px_-5px_rgba(0,0,0,0.35)] hover:border-slate-300 dark:hover:border-slate-700 hover:-translate-y-0.5 transition-all duration-200 text-left border border-slate-200/90 dark:border-slate-800/90 group w-full cursor-pointer relative"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            {drug.blackBox && (
              <span className="text-xs font-bold px-2 py-0.5 rounded-md bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-900/60" title="FDA Boxed Warning">
                ⚠️ Boxed Warning
              </span>
            )}
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">{drug.subgroup}</p>
        </div>

        <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
          {drug.targetDose && (
            <span className="text-xs font-bold bg-slate-100 dark:bg-slate-800/90 text-slate-800 dark:text-slate-200 border border-slate-200/80 dark:border-slate-700/80 px-2.5 py-1 rounded-xl whitespace-nowrap">
              🎯 {drug.targetDose.split('·')[0]}
            </span>
          )}
          {drug.halfLife && (
            <span className="text-xs text-slate-500 dark:text-slate-400 font-medium whitespace-nowrap">
              ⏱️ t½: {drug.halfLife.split('(')[0].trim()}
            </span>
          )}
        </div>
      </div>
      ...
      {drug.indications && drug.indications.length > 0 && (
        <p className="text-sm text-slate-600 dark:text-slate-300 line-clamp-1 mt-2.5 font-normal">
      ...
      {drug.receptors && drug.receptors.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5 mt-3 pt-2.5 border-t border-slate-100 dark:border-slate-800/80">
          {drug.receptors.slice(0, 4).map(r => (
            <span
              key={r.receptor}
              className="text-xs font-semibold px-2.5 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800/80 text-slate-700 dark:text-slate-300 border border-slate-200/60 dark:border-slate-700/60"
            >
              {r.receptor} {r.occupancy ? `${r.occupancy}%` : ''}
            </span>
          ))}
          {drug.receptors.length > 4 && (
            <span className="text-xs text-slate-400 dark:text-slate-500 px-1 py-0.5 font-medium">
